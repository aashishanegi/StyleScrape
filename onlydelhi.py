...
def scrape_justdial_salons(city):
    ...
    if response.status_code == 200:
        ...
        salons = []
        # results = soup.find_all('div', class_='jsx-d444e6bc48e0e7f1 resultbox_info')
...
def save_to_csv(salons, city):
    ...
    with open(filename, mode='w', newline='', encoding='utf-8') as file:
        writer = csv.DictWriter(file, fieldnames=['name', 'address', 'rating', 'photo','timing', 'phone','yib','Services'])
        writer.writeheader()
        for salon in salons:
            writer.writerow(salon)
    print(f"Data saved to {filename}")

# Example usage: Scraping salons in Delhi from Justdial
city = 'Delhi'
...
if salons:
    for idx, salon in enumerate(salons, start=1):
        ...
        # print(f"Photo: {salon['photo']}")
        print(f"Opens at: {salon['timing']}")
        print(f"Phone: {salon['phone']}")
        print(f"Years In Business: {salon['yib']}")
        print(f"Services: {salon['Services']}")
        print("-" * 30)
